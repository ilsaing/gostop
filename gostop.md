...
3. **동일 월 라이벌 여부**(`sameMonthRival`) — 손패 여러 장이 캡처 결과(`taken`)가 완전히 같은 후보(예:
   ...
   - **광박·피박 이중위험 보험**(`gwangDoubleJeopardyInsurance`): 광박(광 0장)과 피박(피 1~5장)이
     ...
     앞당겼다. 몬테카를로 시뮬레이션(`gostop-mcts.html`) 검증으로 확인된 통찰을 `gostop.html`에도 반영한
