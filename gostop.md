...
- `cardWeight(card, playerIdx)`(1687줄): 카드 한 장의 "기본 가치"를 숫자로 환산. (3차 튜닝: 광/쌍피/피/
  ...
  - 띠/열끗(고도리)은 그 조합이 **이 플레이어 본인 기준으로** 아직 완성 가능한지(`ttiComboAchievable`/
    `godoriAchievable`)에 따라 갈림: 가능하면 띠=열끗=2(동일). 이미 죽은 조합이면 1.2(띠·열끗 동일)를
    기본으로 하되, `flatThresholdBonus()`(1713줄 — 5장 이상 평문 점수 문턱에 얼마나 가까운지, 이미 4장
    이상 캡처했으면 +1.4까지)만큼 가산. 9월 유동형 카드는 열끗/쌍피 중 낮은 쪽이라도 최소 쌍피 가치는
    보장(`Math.max(6, asYeol)`).
