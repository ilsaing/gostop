...
5. (라이벌이 아닐 때만) **손패 지원도**(`handSupport`) — 이 캡처 결과가 남는 손패와 타입이 얼마나 겹치는지.
   띠는 단순히 "띠라는 타입"만 같아서는 의미가 없으므로(색이 다르면 단 조합에 전혀 도움 안 됨) 같은
   색(`ttiType`)일 때만 겹치는 것으로 센다 — 예를 들어 12월 비띠(rain, 단 조합 없음)와 5월 초단 띠는
   같은 "띠 타입"이어도 서로 무관하므로 더 이상 시너지로 취급하지 않는다(광/열끗/피는 월·색과 무관하게
   실질적 연관이 있어 그대로 raw 타입 비교를 쓴다).
